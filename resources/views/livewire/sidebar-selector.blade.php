<div>
    <a href="{{ route('admin.all-offers') }}" class="flex flex-row items-center justify-between mb-2 py-2 text-sm font-medium text-sub-accent">
        Ga til platform
            ...
            <flux:menu.radio.group class="h-64 overflow-y-auto">
                @if ( auth()->user()->hasRole('admin') )
                    @foreach (\App\Models\Company::all() as $company)
                        <button type="button" class="flex items-center px-2 py-1.5 w-full focus:outline-hidden rounded-md text-start text-sm font-medium text-zinc-800 data-active:bg-zinc-50 dark:text-white dark:data-active:bg-zinc-600 **:data-flux-menu-item-icon:text-zinc-400 dark:**:data-flux-menu-item-icon:text-white/60" data-flux-menu-item="data-flux-menu-item" 
